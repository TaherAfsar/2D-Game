...
public class Gamelevel2 extends JFrame implements KeyListener {
    ...
    static JLabel bulletLeft;
    static JLabel bulletRight;
    ...
    Gamelevel2() {
        ...
        b=new JButton();
        pb = new JProgressBar();
        bulletLeft = new JLabel();
        bulletRight = new JLabel();
        ...
        Thief.setIcon(new ImageIcon("static/Thief2.png"));
        ...
        bulletLeft.setIcon(new ImageIcon("static/bulletRight.png"));
        bulletRight.setIcon(new ImageIcon("static/bulletLeft.png"));
        ...
        bg.add(Police);
        bg.add(escapeDoor);
        bulletLeft.setBounds(width+500,height+500,100,100);
        bulletRight.setBounds(width+500,height+500,100,100);
        bg.add(bulletLeft);
        bg.add(bulletRight);
    ...
    void checkCollision(JLabel a, JLabel b) {
        ...
        if(!GameOver){
            ...
            if (rectA.intersects(rectB)) {
                ...
                PoliceWins = true;
                GameOver();
                System.out.println("Gamelevel2 OVER.Police WINS!");
            } else {
                moveChar();
                moveChar2();
            }
    ...
    boolean checkCollisionOfBullets(JLabel a, JLabel b) {
        Rectangle rectA = a.getBounds();
        Rectangle rectB = b.getBounds();
        if (rectA.intersects(rectB)) {
            a.setLocation(20000, 20000);
            return true;
        } else {
            return false;
        }
    ...
    void CheckBullet(String direction){


        if(!(Police.getY() > Thief.getY()+5)&& Objects.equals(direction, "left") &&Thief.getX() < Police.getX()){
            System.out.println("HELNA");
            FireBullet(direction);
        }
        else if(!(Police.getY() > Thief.getY()+5) && Objects.equals(direction, "right") &&Thief.getX() > Police.getX()){
    ...
    void FireBullet(String direction){

        if(Objects.equals(direction, "right")){
            System.out.println("HELLO");
            bulletLeft.setLocation(Police.getX()+100,Police.getY());
            Thread animation2 = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    int x = Police.getX();
                    while (true){
                        bulletRight.setLocation(x,bulletRight.getY());
                        boolean hit = checkCollisionOfBullets(bulletRight,Thief);
                        if(hit){
            ...
            Thread animation = new Thread(new Runnable() {
                ...
                public void run() {
                    int x = Police.getX();
                    while (true){
                        bulletLeft.setLocation(x,bulletLeft.getY());
                        boolean hit = checkCollisionOfBullets(bulletLeft,Thief);
                        if(hit){
                            System.out.println("HE GOT HIT" );
                            health = pb.getValue();
                            health=health-30;
